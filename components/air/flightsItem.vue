<template>
    <div class="flight-item">
        ...
        <div v-show="isShow" class="flight-recommend" v-for="(item ,index) in data.seat_infos" :key="index">
            <!-- 隐藏的座位信息列表 -->
...
export default {
    ...
    computed:{
        mistiming(){
            let {dep_time:start,arr_time:end} = this.data
            // 先把时间转换为分钟
            ...
            let time = endMin - startMin
            if(time < 0){
                // 如果是第二天凌晨时间段，需要加24小时
                time = endMin + 24 * 60 - startMin
            }
